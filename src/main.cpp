...
const string strMessageMagic = "VirtacoinPlus Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    int64_t nSubsidy = 30 * COIN;   
      ...
      if (pindexBest->nHeight >= 53965 && pindexBest->nHeight <= 525600)
          ...
          else if (pindexBest->nHeight > 525600 && pindexBest->nHeight < 525600 * 2 )
          {
              nSubsidy = 2.5 * COIN;
          }

          else if (pindexBest->nHeight >= 525600 * 2  && pindexBest->nHeight <= 525600 * 3 )
          {
              nSubsidy = 2.3 * COIN;
          }

          else if (pindexBest->nHeight >= 525600 * 3  && pindexBest->nHeight <= 525600 * 4 )
          {
              nSubsidy = 2.1 * COIN;
          }

          else if (pindexBest->nHeight >= 525600 * 4  && pindexBest->nHeight <= 525600 * 5 )
          {
              nSubsidy = 1.9 * COIN;
          }

          else if (pindexBest->nHeight >= 525600 * 5  && pindexBest->nHeight <= 525600 * 6 )
          {
              nSubsidy = 1.7 * COIN;
          }

          else if (pindexBest->nHeight >= 525600 * 6  )
          {
              nSubsidy = 1.5 * COIN;
          }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VirtacoinPlus", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "VirtacoinPlus New Era";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1488232176;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 86703;
		if(fTestNet)
        {
			block.nTime    = 1445766937;
            block.nNonce   = 14023;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x0bb03c6d37c4bdc8959c823f1ac4116bf755ea611a68c85a71775596554bd17f"));
        block.print();
